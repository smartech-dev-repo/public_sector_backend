...
- **Global default**, applied everywhere via `APP_GUARD`: `THROTTLE_DEFAULT_LIMIT` (default `100`) requests per `THROTTLE_DEFAULT_TTL_SECONDS` (default `60`) per IP — both env-configurable, since `ThrottlerModule.forRootAsync`'s factory genuinely has `ConfigService` available at registration time.
- **Six specific routes** override that same `default` throttler's numbers via `@Throttle({ default: { limit: 5, ttl: 900000 } })` (5 requests per 15 minutes) — a hardcoded provisional pair, not env-configurable like the global default is. `@Throttle()` is a class/method decorator evaluated at module-load time, before Nest's DI container (and so `ConfigService`) exists yet, so it can only take static literal values, not an injected config lookup.

...
- **2FA**: an admin/agent chooses **one** method at setup time — TOTP (authenticator app) or email-based OTP — per your decision. Not both stacked, not TOTP-with-email-fallback.
...
Since Admin and Agent get separate implementations, this section describes the pattern once; §4/§5 show it applied to each.
...
One new shared enum, and the same set of fields added independently to both `AdminUser` and `Agent`:
...
Added to `AdminUser` and to `Agent` (identical field set on each):
...
Identical shape, `/auth/agent/...` instead of `/auth/admin/...`, minus `change-password` (already exists):

| Endpoint | Auth | Notes |
|---|---|---|
| `POST /auth/agent/forgot-password` | Public | Same as Admin |
| `POST /auth/agent/reset-password` | Public | Same as Admin |
| `POST /auth/agent/2fa/setup` | Agent JWT | Same as Admin |
| `POST /auth/agent/2fa/confirm` | Agent JWT | Same as Admin |
| `POST /auth/agent/2fa/disable` | Agent JWT | Same as Admin |
| `POST /auth/agent/login` | Public (existing, modified) | Same as Admin; note the existing `mustChangePassword` forced-change flow is orthogonal to this — a freshly-approved agent with `mustChangePassword: true` and 2FA *not yet set up* logs in normally (2FA only gates login once the agent has actually enabled it, which requires being logged in first) |
| `POST /auth/agent/2fa/login-verify` | Public | Same as Admin |
...
4. **Agent 2FA** — mirrors (2)'s pattern on the `Agent` model, reusing `TokenService`'s already-built pending-token methods from (2).
